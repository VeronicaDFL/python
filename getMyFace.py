...
#To speed up the process,we will use 1/4 resolution on our camera and skip every other frame

video_capture = cv2.VideoCapture(0)# references webcam#0 (default one)

#Load our first sample face:
...
vero_image = face_recognition.load_image_file("vero.jpg")
vero_face_encoding = face_recognition.face_encodings(vero_image)[0]

# create an array of known face encodings and their names:

known_face_encodings = [todd_face_encoding, damian_face_encoding, jiho_face_encoding, vero_face_encoding]
known_face_names = ["Todd A.", "Damian M.", "Jiho S.", "Vero D."]
...
while True: # Loop forever till we quit...
    # grab single framme from webcam
    ...
    for (top,right, bottom, left), name in zip (face_locations, face_names):
        ...
        #Draw  alabel with a name below the face 
